...
**대응/완화책:**
1. **(원칙) PREPARED에서는 자동 Abort 금지:** 참여자는 최종 결정을 모르면 **대기(Blocking)** 하는 것이 2PC의 기본 가정입니다.
2. **3PC (Three-Phase Commit):** Pre-commit 단계를 추가해 블로킹을 줄이려는 시도 (단순성/오버헤드 희생)
3. **휴리스틱(Heuristic) 종료 + 재조정:** 운영자가 직접 `COMMIT PREPARED` 또는 `ROLLBACK PREPARED`를 실행해 강제 종료할 수 있으나, **일관성 깨짐 위험**이 있어 사후 reconciliation 절차가 필요합니다.
...
### 5. 분할 브레인(Split Brain) / 불일치 가능성

**정상 2PC(=PREPARED 이후 자동 결정 없음)**에서는 참여자가 임의로 Commit/Abort를 결정하지 않기 때문에,
네트워크 분할 자체만으로 "참여자들이 서로 다른 결정을 내리는" 형태의 split-brain이 발생하는 것이 아니라,
대개 아래와 같은 **부분 전달 + 휴리스틱 종료(또는 잘못된 운영 개입)**가 결합될 때 불일치가 생깁니다.

**대표 시나리오:**
- 코디네이터가 **Commit을 기록(결정 확정)**하고 일부 참여자에게만 Commit 전달 후 장애
- 다른 참여자들은 PREPARED 상태로 **블로킹**
- 운영/시스템이 무한 대기를 피하려고 일부 참여자에 대해 **휴리스틱 ROLLBACK/ABORT**를 수행

**결과:**
- 일부 참여자는 Commit 완료, 다른 참여자는 Rollback → **불일치(2PC 원자성 붕괴)**

**예방/완화:**
- PREPARED 상태에 대한 **휴리스틱 종료는 최후의 수단**으로만 사용
- 코디네이터의 영구 로그/복구 절차를 강화하고, 필요 시 **새 코디네이터 선출** 시에도 동일 로그를 기준으로 결정을 승계
- 휴리스틱 종료를 허용한다면, 사후 **reconciliation(재조정) 프로세스**를 설계
