...
-   **좀비 프로듀서 차단(Fencing)**: 새로운 프로듀서가 동일한 `transactional.id`로 접속하면, 이전 프로듀서의 요청은 거부됩니다.

### 좀비 프로듀서 차단은 무엇으로 하냐? (Producer Epoch)

결론부터 말하면, Kafka 트랜잭션에서 좀비 프로듀서를 차단하는 핵심은 **Producer Epoch**(프로듀서 에폭) 기반의 **Fencing**입니다.

- 브로커(정확히는 Transaction Coordinator)는 `transactional.id`별로 "현재 유효한 프로듀서"를 식별합니다.
- 동일한 `transactional.id`로 새 프로듀서 인스턴스가 초기화되면, 코디네이터는 **epoch를 증가**시켜 새 인스턴스를 "정상 리더"로 인정합니다.
- 이후 **구(舊) 프로듀서(좀비)**가 Produce/Txn 요청을 보내면, 브로커는 "epoch가 낮다"는 이유로 거부합니다.
  - 클라이언트 관점에서는 보통 `PRODUCER_FENCED` 오류로 관측됩니다.

이 구조는 분산락에서 말하는 **fencing token**과 매우 유사합니다.
- `transactional.id` = 리소스 키
- `producer epoch` = fencing token(단조 증가 토큰)
- "토큰이 낮은 주체의 쓰기"는 거부

#### ASCII로 보는 Fencing 흐름

```
Time →

P_old (transactional.id=T)      Txn Coordinator / Broker           P_new (transactional.id=T)
         |                                  |                               |
         |-- Init(T) ---------------------->|                               |
         |<-- epoch=1, PID=... -------------|                               |
         |                                  |                               |
...
> 요점: PREPARED 이후 자동 결정이 금지되는 2PC와 달리, Kafka 트랜잭션의 "결정"은 코디네이터가 갖고,
> 프로듀서의 중복 실행/재시작에 대해서는 epoch(fencing token)로 "쓰기 권한"을 단조 증가 방식으로 통제합니다.
...
- **KIP-129**: Transactions support in Kafka Streams
